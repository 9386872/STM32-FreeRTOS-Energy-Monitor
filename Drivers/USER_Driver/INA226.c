...
uint8_t INA226_SetConfig1(uint16_t ConfigWord);
uint8_t INA226_SetCalibrationReg1(uint16_t ConfigWord);
uint8_t INA226_SetConfig2(uint16_t ConfigWord);
uint8_t INA226_SetCalibrationReg2(uint16_t ConfigWord);
uint16_t INA226_GetBusVReg1(void);
uint16_t INA226_GetBusVReg2(void);
uint16_t INA226_GetPowerReg1(void);
uint16_t INA226_GetPowerReg2(void);
uint16_t INA226_GetCurrentReg1(void);
uint16_t INA226_GetCurrentReg2(void);

#define INA226_COM_PORT1	hi2c1		/*通讯使用的IIC1接口*/
#define INA226_COM_PORT2	hi2c2		/*通讯使用的IIC1接口*/
...
void INA226_init(void)
{
	...
	*/
    INA226_SetConfig1(0x45FF);
	INA226_SetConfig2(0x45FF);
	/*
	...
	*/
    INA226_SetCalibrationReg1(0x0a00);
	INA226_SetCalibrationReg2(0x0a00);
...
**************************************************
*/
float INA226_GetCurrent1()
{
	uint16_t regData;
	float fCurrent;
	regData = INA226_GetCurrentReg1();
	if(regData >= 0x8000)	regData = 0;
	fCurrent = regData * 0.0002f;/*电流的LSB = 0.2mA，由用户配置*/
	return fCurrent;
}

float INA226_GetCurrent2()
{
	uint16_t regData;
	float fCurrent;
	regData = INA226_GetCurrentReg2();
	if(regData >= 0x8000)	regData = 0;
	fCurrent = regData * 0.0002f;/*电流的LSB = 0.2mA，由用户配置*/
	return fCurrent;
}
...
**************************************************
*/
float INA226_GetPower1()
{
	uint16_t regData;
	float fPower;
	regData = INA226_GetPowerReg1();
	fPower = regData * 0.005f;/*功率的LSB = 电流的LSB*25*/
	return fPower;
}
float INA226_GetPower2()
{
	uint16_t regData;
	float fPower;
	regData = INA226_GetPowerReg2();
	fPower = regData * 0.005f;/*功率的LSB = 电流的LSB*25*/
	return fPower;
}
uint8_t INA226_SetConfig1(uint16_t ConfigWord)
{
    ...
    return HAL_I2C_Master_Transmit(&INA226_COM_PORT1, INA226_ADDRESS, SentTable, 3, INA226_I2C_TIMEOUT);
}
...
uint8_t INA226_SetConfig2(uint16_t ConfigWord)
{
    ...
    SentTable[1] = (ConfigWord & 0xFF00) >> 8;
    SentTable[2] = (ConfigWord & 0x00FF);
	return HAL_I2C_Master_Transmit(&INA226_COM_PORT2, INA226_ADDRESS, SentTable, 3, INA226_I2C_TIMEOUT);
}
...
uint16_t INA226_GetBusVReg1()
{
    uint8_t SentTable[1] = {INA226_BUSV};
    uint8_t ReceivedTable[2];
    HAL_I2C_Master_Transmit(&INA226_COM_PORT1,INA226_ADDRESS, SentTable, 1, INA226_I2C_TIMEOUT);
    if (HAL_I2C_Master_Receive(&INA226_COM_PORT1,INA226_ADDRESS, ReceivedTable, 2, INA226_I2C_TIMEOUT) != HAL_OK) return 0xFF;
    else return ((uint16_t)ReceivedTable[0]<<8 | ReceivedTable[1]);
}

uint16_t INA226_GetBusVReg2()
{
    uint8_t SentTable[1] = {INA226_BUSV};
    uint8_t ReceivedTable[2];
    HAL_I2C_Master_Transmit(&INA226_COM_PORT2,INA226_ADDRESS, SentTable, 1, INA226_I2C_TIMEOUT);
    if (HAL_I2C_Master_Receive(&INA226_COM_PORT2,INA226_ADDRESS, ReceivedTable, 2, INA226_I2C_TIMEOUT) != HAL_OK) return 0xFF;
    else return ((uint16_t)ReceivedTable[0]<<8 | ReceivedTable[1]);
}
...
uint16_t INA226_GetCurrentReg1()
{
...
uint16_t INA226_GetCurrentReg2()
{
    uint8_t SentTable[1] = {INA226_CURRENT};
    uint8_t ReceivedTable[2];
    HAL_I2C_Master_Transmit(&INA226_COM_PORT2,INA226_ADDRESS, SentTable, 1, INA226_I2C_TIMEOUT);
